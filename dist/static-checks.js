 ...
 */
export function lintValidatedSuite(suite, capabilities) {
    const diagnostics = [];
    function emit(ruleId, message, pointer, questionId) {
        diagnostics.push({
    ...
    function duplicates(ids, pointer) {
        const seen = new Set();
        ids.forEach((id, index) => {
            if (seen.has(id))
                emit("QCT008", `Duplicate identifier: ${id}`, `${pointer}/${index}`);
            seen.add(id);
        });
    }
    ...
    suite.state.stages.forEach((stage, index) => {
        for (const parent of stage.after) {
            if (!stages.has(parent))
                emit("QCT002", `Unknown stage: ${parent}`, `/state/stages/${index}/after`);
        }
    });
    suite.state.fields.forEach((field, index) => {
        ...
        for (const parent of field.derivedFrom) {
            if (!fields.has(parent))
                emit("QCT002", `Unknown source field: ${parent}`, `/state/fields/${index}/derivedFrom`);
        }
    });
    ...
    suite.questions.forEach((question, index) => {
        ...
        if (question.prediction) {
            ...
            if (!target)
                emit("QCT002", "Unknown prediction target.", `${pointer}/prediction`, question.id);
    ...
    suite.bindings.forEach((binding, index) => {
        ...
        if (!questions.has(binding.questionId))
            emit("QCT002", `Unknown bound question: ${binding.questionId}`, pointer);
        if (!stages.has(binding.atStage))
            emit("QCT002", `Unknown binding stage: ${binding.atStage}`, pointer, binding.questionId);
        ...
        binding.gates.forEach((gate, gateIndex) => {
            ...
            if (!other)
                emit("QCT002", `Unknown gate question: ${gate.questionId}`, gatePointer, binding.questionId);
            else if (other.output.kind !== "boolean")
                emit("QCT007", "A gate must refer to a boolean question.", gatePointer, binding.questionId);
            if (gate.falseAtMost >= gate.trueAtLeast)
                emit("QCT007", "Gate thresholds must leave a non-empty undecided interval.", gatePointer, binding.questionId);
            if (purposes.has(gate.purpose))
                emit("QCT007", `Duplicate gate purpose: ${gate.purpose}`, gatePointer, binding.questionId);
