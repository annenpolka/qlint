...
評価メモ（2026-09-18）: 80ケース（ベース60 + 改訂前に先書きしたheld-out 20 = eval2）をliveで測定した。v6時点で全体49/55検出、正当例停止24%、根拠span一致84%。held-outのeval2は12/15検出、正当例停止20%、span 100%。rule別ではQBE004が完璧な一方、QSM002の過剰発火（fp 28）が最大の課題。**これはルール品質の測定結果であり、検出器の完成宣言ではない。** 改訂はtuning分割のみで判断し、eval2は凍結後の初観測として一度だけ実行した。次の改訂では新しいheld-outを追加する。
